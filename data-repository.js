...
class NorthstarDemoRepository {
  ...
  async createJob(customerId, service, time, appointment = {}) {
    ...
    const response = await fetch('/api/jobs', { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json', 'idempotency-key': crypto.randomUUID() }, body: JSON.stringify({ customerId, service, time, ...(appointment.requiredSkill ? { requiredSkill: appointment.requiredSkill } : {}), ...(appointment.startsAt ? { startsAt: appointment.startsAt } : {}), ...(appointment.endsAt ? { endsAt: appointment.endsAt } : {}), ...(appointment.timeZone ? { timeZone: appointment.timeZone } : {}) }) });
