...
class NorthstarDemoRepository {
  ...
  async createCustomReportView(name, metrics, startDate = '', endDate = '', filters = {}, groupBy = '', idempotencyKey = crypto.randomUUID()) {
    if (!this.remote) throw new Error('API required for report views');
    const response = await fetch('/api/reports/custom/views', { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json', 'idempotency-key': idempotencyKey }, body: JSON.stringify({ name, metrics, startDate, endDate, filters, groupBy }) });
  ...
  async getTechnicianScorecards(range = {}) {
    if (!this.remote) throw new Error('API required for technician scorecards');
    const query = new URLSearchParams(); if (range?.startDate) query.set('startDate', String(range.startDate)); if (range?.endDate) query.set('endDate', String(range.endDate));
    const response = await fetch(`/api/reports/technician-scorecards${query.toString() ? `?${query}` : ''}`, { headers: { authorization: `Bearer ${this.token}` } });
    if (!response.ok) throw new Error('technician scorecards unavailable');
    return response.json();
  }
  ...
  async getOperationalMetrics() {
    ...
    if (!response.ok) throw new Error('operational metrics unavailable');
    return response.json();
  }
  ...
  async dispatchAccounting(limit = 20) {
    if (!this.remote) throw new Error('API required for accounting dispatch');
    const response = await fetch('/api/integrations/accounting/dispatch', { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json' }, body: JSON.stringify({ limit }) });
    if (!response.ok) throw new Error((await response.json().catch(() => ({}))).error || 'accounting dispatch failed');
    return response.json();
  }
  ...
  async retryPayrollRun(id) {
    if (!this.remote) throw new Error('API required for payroll retry');
    const response = await fetch('/api/integrations/payroll/retry', { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json' }, body: JSON.stringify({ payrollRunId: id }) });
    if (!response.ok) throw new Error((await response.json().catch(() => ({}))).error || 'payroll retry failed');
    return response.json();
  }
  ...
  async dispatchDocuments(limit = 20) {
    if (!this.remote) throw new Error('API required for document dispatch');
    const response = await fetch('/api/integrations/documents/dispatch', { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json' }, body: JSON.stringify({ limit }) });
    if (!response.ok) throw new Error((await response.json().catch(() => ({}))).error || 'document dispatch failed');
    return response.json();
  }
  ...
  async completeJob(id, note, idempotencyKey = crypto.randomUUID()) {
    if (!this.remote) throw new Error('API required for job completion');
    const response = await fetch(`/api/jobs/${encodeURIComponent(id)}/complete`, { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json', 'idempotency-key': idempotencyKey }, body: JSON.stringify({ note }) });
  ...
  async getAssetHistory(id) {
    ...
    if (!response.ok) throw new Error('asset history failed');
    return response.json();
  }

  async lookupAsset(code) {
    if (!this.remote) throw new Error('API required for asset lookup');
    const response = await fetch(`/api/assets/lookup?code=${encodeURIComponent(code)}`, { headers: { authorization: `Bearer ${this.token}` } });
    if (!response.ok) throw new Error('asset lookup failed');
    return response.json();
  }
  ...
  async createPlan(customerId, service, amount, renewal, idempotencyKey = crypto.randomUUID(), assetId = '', catalogItemId = '', billingSchedule = 'Monthly', options = {}) {
    if (!this.remote) throw new Error('API required for service plan creation');
    const response = await fetch('/api/plans', { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json', 'idempotency-key': idempotencyKey }, body: JSON.stringify({ customerId, service, amount, renewal, billingSchedule, ...(options.autoRenew !== undefined ? { autoRenew: options.autoRenew } : {}), ...(options.renewalAt ? { renewalAt: options.renewalAt } : {}), ...(options.visitsIncluded !== undefined ? { visitsIncluded: options.visitsIncluded } : {}), ...(assetId ? { assetId } : {}), ...(catalogItemId ? { catalogItemId } : {}) }) });
    if (!response.ok) throw new Error('service plan creation failed');
    return response.json();
  }

  async updatePlanStatus(id, action, note = '', idempotencyKey = crypto.randomUUID()) {
    if (!this.remote) throw new Error('API required for service plan status updates');
    const response = await fetch(`/api/plans/${encodeURIComponent(id)}/${action}`, { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json', 'idempotency-key': idempotencyKey }, body: JSON.stringify({ ...(note ? { note } : {}) }) });
  ...
  async resendUserInvite(id, idempotencyKey = crypto.randomUUID()) {
    if (!this.remote) throw new Error('API required for user invitations');
    const response = await fetch(`/api/users/invites/${encodeURIComponent(id)}/resend`, { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json', 'idempotency-key': idempotencyKey }, body: '{}' });
    if (!response.ok) throw new Error((await response.json().catch(() => ({}))).error || 'user invitation resend failed');
    return response.json();
  }
  ...
  async listCustomerPricing(customerId) {
    if (!this.remote) throw new Error('API required for customer pricing');
    const response = await fetch(`/api/customers/${encodeURIComponent(customerId)}/pricing`, { headers: { authorization: `Bearer ${this.token}` } });
    if (!response.ok) throw new Error('customer pricing unavailable');
    return response.json();
  }

  async setCustomerPricing(customerId, catalogItemId, amount, idempotencyKey = crypto.randomUUID()) {
    if (!this.remote) throw new Error('API required for customer pricing');
    const response = await fetch(`/api/customers/${encodeURIComponent(customerId)}/pricing`, { method: 'POST', headers: { authorization: `Bearer ${this.token}`, 'content-type': 'application/json', 'idempotency-key': idempotencyKey }, body: JSON.stringify({ catalogItemId, amount }) });
    if (!response.ok) throw new Error('customer pricing update failed');
    return response.json();
  }
  ...
  async getCallRecording(callId) {
    ...
    const response = await fetch(`/api/calls/${encodeURIComponent(callId)}/recording`, { headers: { authorization: `Bearer ${this.token}` } });
    if (!response.ok) throw new Error((await response.json().catch(() => ({}))).error || 'call recording unavailable');
    return response.json();
  }
