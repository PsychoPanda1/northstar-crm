...
const viewTitles = { customers: 'Customers', leads: 'Lead inbox', estimates: 'Estimates', invoices: 'Invoices', plans: 'Service plans', activities: 'Customer timeline', dispatch: 'Dispatch board', team: 'Team roster', catalog: 'Service catalog', notifications: 'Notifications' };
...
document.querySelector('#export-records').addEventListener('click', async () => { const type = drawer.dataset.view; if (!type || ['reports', 'team', 'catalog', 'notifications'].includes(type)) { showToast('Open a customer, lead, billing, plan, activity, or dispatch view first.'); return; } try { await repository.exportRecords(type); showToast(`${viewTitles[type]} exported as CSV.`); } catch { showToast('Could not export this workspace.'); } });
