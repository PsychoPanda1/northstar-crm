...
document.querySelector('#drawer-refresh').addEventListener('click', () => { if (drawer.dataset.view) openRecords(drawer.dataset.view); });
recordSearch.addEventListener('input', async () => { const active = drawer.dataset.view; if (!active) return; try { renderRecords(await repository.list(active, recordSearch.value), active); } catch {} });
}
bootstrap();
