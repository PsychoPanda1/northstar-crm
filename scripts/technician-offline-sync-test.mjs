...
try {
  ...
  if (!login.response.ok || link.response.status !== 200 || batch.response.status !== 200 || batch.body.duplicate !== false || batch.body.results?.length !== 2 || batch.body.results.some((item) => !item.ok) || duplicate.response.status !== 200 || duplicate.body.duplicate !== true || persisted.jobs[0].status !== 'In progress' || persisted.jobs[0].checklist?.[0]?.completed !== true || persisted.technicianSyncBatches?.length !== 1) throw new Error('bounded technician offline sync did not apply and persist idempotent actions');
