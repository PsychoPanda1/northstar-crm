...
try {
  ...
  const failedBatchId = `SYNC-FAILED-${Date.now()}`;
  const failedBatch = await post(`/api/public/technician-job/offline-sync?token=${encodeURIComponent(token)}`, { batchId: failedBatchId, actions: [{ id: 'ACTION-FAILED-1', path: '/api/public/technician-job/checklist', method: 'POST', body: { index: 99, completed: true }, idempotencyKey: 'offline-failed-1' }] });
  const retry = await post(`/api/public/technician-job/offline-sync/retry?token=${encodeURIComponent(token)}`, { batchId: failedBatchId, actionIds: ['ACTION-FAILED-1'] });
  const review = await request('/api/technician/offline-sync/review', { headers: auth });
  const notifications = await request('/api/notifications?search=offline%20sync', { headers: auth });
  const discard = await post('/api/technician/offline-sync/discard', { batchId: failedBatchId, actionIds: ['ACTION-FAILED-1'], note: 'Verified duplicate field update on the owner console.' }, auth);
  const retryReviewBatchId = (review.body.batches || []).find((item) => item.batchId !== failedBatchId && item.failedActions?.some((action) => action.id === 'ACTION-FAILED-1'))?.batchId;
  const discardRetry = retryReviewBatchId ? await post('/api/technician/offline-sync/discard', { batchId: retryReviewBatchId, actionIds: ['ACTION-FAILED-1'], note: 'Resolved the retried duplicate field update.' }, auth) : { response: { status: 200 }, body: {} };
  const resolvedNotifications = await request('/api/notifications?search=offline%20sync', { headers: auth });
  const persisted = JSON.parse(readFileSync(dataFile, 'utf8'))[tenantId];
  const failedBatchRecord = persisted.technicianSyncBatches?.find((item) => item.batchId === failedBatchId);
  if (!login.response.ok || link.response.status !== 200 || batch.response.status !== 200 || batch.body.duplicate !== false || batch.body.results?.length !== 2 || batch.body.results.some((item) => !item.ok) || duplicate.response.status !== 200 || duplicate.body.duplicate !== true || history.response.status !== 200 || history.body.batches?.length !== 1 || history.body.batches[0].results?.length !== 2 || history.body.batches[0].results.some((item) => item.body || item.idempotencyKey) || failedBatch.response.status !== 200 || failedBatch.body.results?.[0]?.ok !== false || retry.response.status !== 200 || retry.body.results?.[0]?.ok !== false || review.response.status !== 200 || !review.body.batches?.some((item) => item.failedActions?.some((action) => action.id === 'ACTION-FAILED-1')) || discard.response.status !== 200 || discard.body.resolved !== true || discardRetry.response.status !== 200 || notifications.response.status !== 200 || !notifications.body.items?.some((item) => item.title === 'Technician offline sync needs review' && item.batchId) || resolvedNotifications.response.status !== 200 || resolvedNotifications.body.items?.some((item) => item.title === 'Technician offline sync needs review' && item.batchId) || persisted.jobs[0].status !== 'In progress' || persisted.jobs[0].checklist?.[0]?.completed !== true || persisted.technicianSyncBatches?.length !== 3 || failedBatchRecord?.failedActions?.length !== 0 || failedBatchRecord?.discardedActions?.[0]?.id !== 'ACTION-FAILED-1' || !failedBatchRecord?.discardedActions?.[0]?.note) throw new Error('bounded technician offline sync did not apply, encrypt, retry, expose safe receipts, or support owner conflict resolution');
