...
try {
  writeFileSync(dataFile, JSON.stringify({ [tenantId]: { customers: [{ id: 'preferences-customer', tenantId, name: 'Preferences Customer', phone: '8435550111', contactPreferences: { smsOptOut: false, emailOptOut: false } }, { id: 'legacy-retry-customer', tenantId, name: 'Legacy Retry Customer', phone: '8435550333', location: '3 Legacy Lane', customerIdempotencyKey: 'legacy-create-retry' }], auditEvents: [], activities: [] } }));
  ...
  const legacyHeaders = { ...auth, 'content-type': 'application/json', 'idempotency-key': 'legacy-create-retry' };
  const legacyConflict = await request('/api/customers', { method: 'POST', headers: legacyHeaders, body: JSON.stringify({ name: 'Changed Legacy Customer', phone: '8435550333', location: '3 Legacy Lane' }) });
