...
try {
  ...
  const lateBounce = await signed({ eventId: 'delivery-event-late-bounce', tenantId: 'clearwater-plumbing', messageId: 'delivery_sent', status: 'bounced', reason: 'carrier rejected the destination' });
  const staleSuccess = await signed({ eventId: 'delivery-event-stale-success', tenantId: 'clearwater-plumbing', messageId: 'delivery_sent', status: 'delivered' });
  ...
  if (delivered.response.status !== 200 || delivered.body.message?.status !== 'Sent' || delivered.body.message?.providerDeliveryStatus !== 'delivered' || duplicate.response.status !== 200 || !duplicate.body.duplicate || conflict.response.status !== 409 || conflict.body.error !== 'webhook_event_reused' || lateBounce.response.status !== 200 || lateBounce.body.message?.status !== 'Failed' || lateBounce.body.message?.providerDeliveryStatus !== 'bounced' || staleSuccess.response.status !== 409 || staleSuccess.body.error !== 'message_not_pending' || bounced.response.status !== 200 || bounced.body.message?.status !== 'Failed' || bounced.body.message?.providerDeliveryStatus !== 'bounced' || failed.providerError !== 'mailbox unavailable' || sent.providerError !== 'carrier rejected the destination') throw new Error('message delivery webhook status, rotation, or replay contract failed');
