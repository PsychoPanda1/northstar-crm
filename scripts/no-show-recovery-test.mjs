...
try {
  ...
  const rescheduleSlot = availability.body.slotOptions?.[0];
  const rescheduleHeaders = { authorization: `Bearer ${login.body.token}`, 'idempotency-key': 'dispatch-reschedule-1' };
  ...
  const slot = availability.body.slotOptions?.[1];
