...
try {
  ...
  const health = await request('/api/integrations/health', { headers: { authorization: headers.authorization } });
  const saved = JSON.parse(readFileSync(dataFile, 'utf8'))[tenantId];
  const invoice = saved.invoices.find((item) => item.id === 'refund-invoice');
  if (!login.response.ok || first.response.status !== 201 || first.body.invoice?.status !== 'Partially paid' || first.body.invoice?.balance !== 25 || duplicate.response.status !== 200 || !duplicate.body.duplicate || second.response.status !== 201 || rotatedWebhook.response.status !== 201 || health.response.status !== 200 || health.body.checks?.webhookRotation !== true || health.body.checks?.financingWebhookSecret !== false || health.body.webhookRotation?.previousSecrets?.message !== true || invoice.status !== 'Due' || invoice.paidAmount !== 0 || invoice.balance !== 100 || saved.payments.filter((item) => item.refundOf === 'refund-payment').length !== 2) throw new Error('payment refund lifecycle, idempotency, or webhook secret health failed');
