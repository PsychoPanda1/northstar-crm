...
if (!app.includes('import-catalog-csv') || !app.includes('parseCsv') || !app.includes('repository.importCatalogItems(items)') || !app.includes('bulk-catalog-archive') || !app.includes('bulk-catalog-restore') || !app.includes('repository.bulkUpdateCatalogItems(ids, false)') || !app.includes('repository.bulkUpdateCatalogItems(ids, true)')) throw new Error('catalog workspace must expose practical CSV import and bulk archive/restore controls');
