...
try {
  ...
  const update = { businessName: 'Clearwater Plumbing Group', serviceLabel: 'Emergency Plumbing', timeZone: 'America/New_York', appointmentMinutes: 75, intakeFields: [{ id: 'issue_type', type: 'select', label: 'Issue type', required: true, options: ['Leak', 'Clog', 'Other'] }] };
  ...
  assert(updated.response.status === 200 && updated.body.workspace?.businessName === update.businessName && updated.body.workspace?.appointmentMinutes === 75 && updated.body.workspace?.intakeFields?.[0]?.id === 'issue_type', 'workspace settings update was not applied');
  assert(duplicate.response.status === 200 && duplicate.body.duplicate === true, 'workspace settings update was not idempotent');
  assert(manifest.response.status === 200 && manifest.body.tenant?.businessName === update.businessName && manifest.body.tenant?.serviceLabel === update.serviceLabel && manifest.body.tenant?.appointmentMinutes === 75, 'public landing manifest did not reflect workspace settings');
