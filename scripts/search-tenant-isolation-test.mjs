...
try {
  ...
  if (plumbingCustomer.response.status !== 201 || electricalCustomer.response.status !== 201 || lead.response.status !== 201 || search.response.status !== 200 || search.body.results?.customers?.[0]?.name !== 'Cross-tenant-ordinary-match customer') throw new Error('cross-tenant search branch changed current-tenant results');
