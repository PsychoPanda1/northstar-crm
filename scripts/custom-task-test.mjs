...
try {
  ...
  const reminderJob = await post('/api/jobs', { customerId: customers.body.items?.[0]?.id, service: 'Reminder test', time: availability.body.slotOptions?.[0]?.label, slotId: availability.body.slotOptions?.[0]?.id }, { authorization: `Bearer ${login.body.token}`, 'idempotency-key': 'job-reminder-create-once' });
  ...
  const notifyHeaders = { authorization: `Bearer ${login.body.token}`, 'content-type': 'application/json', 'idempotency-key': 'job-notify-once' };
  const notified = await request(`/api/jobs/${job.id}/notify`, { method: 'POST', headers: notifyHeaders, body: JSON.stringify({ template: 'confirmation', channel: 'SMS' }) });
  const notifiedDuplicate = await request(`/api/jobs/${job.id}/notify`, { method: 'POST', headers: notifyHeaders, body: JSON.stringify({ template: 'confirmation', channel: 'SMS' }) });
  const notifiedConflict = await request(`/api/jobs/${job.id}/notify`, { method: 'POST', headers: notifyHeaders, body: JSON.stringify({ template: 'en_route', channel: 'SMS' }) });
  const remindHeaders = { authorization: `Bearer ${login.body.token}`, 'content-type': 'application/json', 'idempotency-key': 'job-remind-once' };
  const reminded = await request(`/api/jobs/${job.id}/remind`, { method: 'POST', headers: remindHeaders, body: JSON.stringify({ channel: 'Email' }) });
  const remindedDuplicate = await request(`/api/jobs/${job.id}/remind`, { method: 'POST', headers: remindHeaders, body: JSON.stringify({ channel: 'Email' }) });
  const remindedConflict = await request(`/api/jobs/${job.id}/remind`, { method: 'POST', headers: remindHeaders, body: JSON.stringify({ channel: 'SMS' }) });
  const bulkHeaders = { authorization: `Bearer ${login.body.token}`, 'content-type': 'application/json', 'idempotency-key': 'bulk-reminders-once' };
  const bulkReminders = await request('/api/dispatch/reminders', { method: 'POST', headers: bulkHeaders, body: JSON.stringify({ hours: 168, channel: 'Email' }) });
  const bulkRemindersDuplicate = await request('/api/dispatch/reminders', { method: 'POST', headers: bulkHeaders, body: JSON.stringify({ hours: 168, channel: 'Email' }) });
  const bulkRemindersConflict = await request('/api/dispatch/reminders', { method: 'POST', headers: bulkHeaders, body: JSON.stringify({ hours: 24, channel: 'Email' }) });
  if (created.response.status !== 201 || duplicate.response.status !== 200 || !duplicate.body.duplicate || !listed.body.items.some((item) => item.id === created.body.task.id) || completed.response.status !== 200 || completed.body.task.status !== 'Completed' || completedDuplicate.response.status !== 200 || !completedDuplicate.body.duplicate || completedConflict.response.status !== 409 || after.body.items.some((item) => item.id === created.body.task.id) || !history.body.items.some((item) => item.id === created.body.task.id && item.status === 'Completed') || technicianTasks.response.status !== 403 || jobCreated.response.status !== 201 || reminderJob.response.status !== 201 || checklist.response.status !== 200 || checklistDuplicate.response.status !== 200 || !checklistDuplicate.body.duplicate || checklistConflict.response.status !== 409 || forms.response.status !== 200 || formsDuplicate.response.status !== 200 || !formsDuplicate.body.duplicate || formsConflict.response.status !== 409 || notified.response.status !== 201 || notifiedDuplicate.response.status !== 200 || !notifiedDuplicate.body.duplicate || notifiedConflict.response.status !== 409 || reminded.response.status !== 201 || remindedDuplicate.response.status !== 200 || !remindedDuplicate.body.duplicate || remindedConflict.response.status !== 409 || bulkReminders.response.status !== 200 || bulkRemindersDuplicate.response.status !== 200 || !bulkRemindersDuplicate.body.duplicate || bulkRemindersConflict.response.status !== 409) throw new Error('custom task, job configuration, notification, or bulk reminder lifecycle failed');
