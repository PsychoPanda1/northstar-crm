...
try {
  writeFileSync(dataFile, JSON.stringify({ [tenantId]: { customers: [{ id: 'location-customer', tenantId, name: 'Location Customer', phone: '843-555-0199', location: '1 Tracking Way' }], jobs: [{ id: 'JOB-LOCATION-1', tenantId, customerId: 'location-customer', customer: 'Location Customer', service: 'Plumbing', technician: 'Alex Rivera', status: 'En route', time: 'Today 9:00 AM', location: '1 Tracking Way' }] } }));
  ...
  if (!login.response.ok || link.response.status !== 200 || first.response.status !== 200 || second.response.status !== 200 || history.response.status !== 200 || history.body.tracking?.status !== 'live' || history.body.pings?.length !== 2 || history.body.pings[1].latitude !== 32.79 || persisted.jobs[0].locationPings?.length !== 2) throw new Error('bounded location history workflow failed');
