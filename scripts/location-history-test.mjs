...
try {
  ...
  const fieldGuide = token ? await request('/api/public/technician-job/field-guide?token=' + encodeURIComponent(token)) : { response: { ok: false }, body: {} };
  const etaUpdate = token ? await post('/api/public/technician-job/status?token=' + encodeURIComponent(token), { status: 'En route', etaMinutes: 25 }) : { response: { ok: false }, body: {} };
  const etaJob = token ? await request('/api/public/technician-job?token=' + encodeURIComponent(token)) : { response: { ok: false }, body: {} };
  ...
  if (!login.response.ok || link.response.status !== 200 || technicianJob.response.status !== 200 || fieldGuide.response.status !== 200 || etaUpdate.response.status !== 200 || etaJob.body.estimatedArrivalMinutes !== 25 || !persisted.messages.some((item) => item.template === 'en_route' && item.message.includes('25 minutes')) || fieldGuide.body.title !== 'Plumbing diagnostic guide' || fieldGuide.body.steps?.length !== 3 || technicianJob.body.materials?.[0]?.sku !== 'TRACK-SKU' || technicianJob.body.materials?.[0]?.barcode !== 'TRACK-BARCODE' || technicianJob.body.preJobBrief?.priorities?.[0] !== 'High priority appointment' || technicianJob.body.preJobBrief?.knownAssets?.[0]?.name !== 'Water heater' || technicianJob.body.preJobBrief?.openFindings?.[0]?.title !== 'Aging shutoff' || technicianJob.body.preJobBrief?.intakeContext?.[0] !== 'problem: Low water pressure' || technicianJob.body.preJobBrief?.recommendedChecks?.length !== 2 || first.response.status !== 200 || second.response.status !== 200 || history.response.status !== 200 || history.body.tracking?.status !== 'live' || history.body.pings?.length !== 2 || history.body.pings[1].latitude !== 32.79 || persisted.jobs[0].locationPings?.length !== 2 || persisted.jobs[0].estimatedArrivalMinutes !== 25) throw new Error('bounded location history, ETA, or safe material identifier workflow failed');
