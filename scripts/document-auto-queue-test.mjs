...
try {
  ...
  writeFileSync(dataFile, JSON.stringify({ [tenantId]: { customers: [{ id: 'customer-auto-queue', tenantId, name: 'Auto Queue Customer' }], estimates: [{ id: 'EST-auto-queue-1', tenantId, customerId: 'customer-auto-queue', customer: 'Auto Queue Customer', service: 'Water heater repair', value: '$400.00', amount: 400, status: 'Sent' }], messages: [{ id: 'MSG-auto-queue-1', tenantId, estimateId: 'EST-auto-queue-1', customerId: 'customer-auto-queue', customer: 'Auto Queue Customer', template: 'estimate_sent', publicEstimateUrl: 'https://crm.example/estimate', publicEstimatePdfUrl: 'https://crm.example/estimate.pdf', status: 'Queued (provider pending)' }] } }));
  ...
  if (!login.response.ok || !run.response.ok || !queued || queued.status !== 'Pending provider' || queued.artifactUrl !== 'https://crm.example/estimate' || queued.pdfUrl !== 'https://crm.example/estimate.pdf' || dispatch.body.delivered !== 1 || received !== 1) throw new Error('automatic document queue materialization failed');
