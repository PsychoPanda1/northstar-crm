...
try {
  ...
  if (!ready?.response?.ok || ready.body.checks?.configuration !== true || ready.body.checks?.tenantDeploymentContract !== true || ready.body.checks?.backupConfiguration !== true || ready.body.checks?.backupStorage !== true || ready.body.checks?.backupSnapshot !== true || ready.body.checks?.storageConfiguration !== true || ready.body.checks?.requestResponseSlaConfiguration !== true || ready.body.checks?.documentRetryConfiguration !== true || ready.body.checks?.payrollRetryConfiguration !== true || ready.body.failedChecks?.length !== 0 || ready.body.issues?.length !== 0) throw new Error(`production server did not become ready with valid configuration: ${JSON.stringify(ready.body)}`);
  const openapiResponse = await fetch(`${base}/api/openapi.yaml`);
  const openapiText = (await openapiResponse.text()).replace(/\r\n/g, '\n');
  if (!openapiResponse.ok || !openapiResponse.headers.get('content-type')?.includes('application/yaml') || !openapiText.includes('openapi: 3.0.3') || !openapiText.includes('/api/public/bookings:') || !openapiText.includes('/api/public/customer-portal/service-plan-visit:') || !openapiText.includes('servicePlanVisit: { type: string }') || !openapiText.includes('required: [action, note]') || !openapiText.includes('planId: { type: string, maxLength: 120, description: Preferred durable plan identifier. }') || !openapiText.includes('        integration:\n          type: object\n          required: [version, leadEndpoint, bookingEndpoint, availabilityEndpoint, ownerAuthMethods]\n          properties:') || !openapiText.includes('/api/session/services:') || !openapiText.includes('/api/calls/{callId}/recording/revoke:') || !openapiText.includes('Revoke call-recording access and remove the stored provider URL') || !openapiText.includes('bearerAuth: []')) throw new Error('canonical OpenAPI endpoint was not served with authenticated workspace discovery');
  ...
  if (!invalidReady || invalidReady.response.status !== 503 || invalidReady.body.checks?.requestResponseSlaConfiguration !== false || invalidReady.body.checks?.rateLimitConfiguration !== false || invalidReady.body.checks?.identityProviderConfiguration !== false || invalidReady.body.checks?.allowedOriginsConfiguration !== false || invalidReady.body.checks?.serviceOriginConfiguration !== false || invalidReady.body.checks?.tenantDeploymentContract !== false || invalidReady.body.checks?.backupConfiguration !== true || invalidReady.body.checks?.backupStorage !== true || invalidReady.body.checks?.tenantDataIntegrity !== false || !invalidReady.body.failedChecks?.includes('requestResponseSlaConfiguration') || !invalidReady.body.issues?.some((item) => item.key === 'requestResponseSlaConfiguration' && item.message)) throw new Error('invalid production configuration did not fail readiness with actionable issues');
  ...
  if (!outboundUrlReady || outboundUrlReady.response.status !== 503 || outboundUrlReady.body.checks?.publicUrlConfiguration !== false || outboundUrlReady.body.checks?.fleetWebhookSecret !== false) throw new Error('provider-backed delivery did not reject unsafe production readiness configuration');
