...
const provider = createServer((req, res) => { providerCalls += 1; lastAuthorization = String(req.headers.authorization || ''); res.writeHead(providerCalls === 1 ? 503 : 200, { 'content-type': 'application/json' }); res.end(JSON.stringify(providerCalls === 1 ? { error: 'temporary outage' } : { status: 'sent', providerReference: 'retry-provider-2' })); });
const env = { ...process.env, NODE_ENV: 'development', PORT: String(port), NORTHSTAR_DATA_FILE: dataFile, NORTHSTAR_SESSION_FILE: sessionFile, NORTHSTAR_MESSAGE_PROVIDER_URL: `http://127.0.0.1:${providerPort}`, NORTHSTAR_MESSAGE_PROVIDER_API_KEY: 'global-message-key', NORTHSTAR_TENANT_PROVIDER_CONFIG_JSON: JSON.stringify({ 'clearwater-plumbing': { message: { url: `http://127.0.0.1:${providerPort}/tenant-messages`, apiKey: 'tenant-message-key' } } }), NORTHSTAR_MESSAGE_RETRY_LIMIT: '1' };
