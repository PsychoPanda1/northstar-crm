...
try {
  ...
  const conflict = await post('/api/dispatch/route-optimize', { date, technician: 'Marcus Thompson', startLatitude: 32.8000, startLongitude: -79.9000, travelSpeedKph: 60 }, { authorization: `Bearer ${login.body.token}`, 'idempotency-key': 'route-conflict-1' });
  ...
  if (!login.response.ok || optimized.response.status !== 200 || optimized.body.optimizedStops !== 3 || optimized.body.skippedStops !== 1 || optimized.body.method !== 'coordinate_nearest_neighbor_2opt_time_safe' || optimized.body.timeWindowFeasible !== true || !Number.isFinite(optimized.body.distanceKm) || duplicate.response.status !== 200 || !duplicate.body.duplicate || conflict.response.status !== 409 || conflict.body.error !== 'route_time_window_conflict' || mapped.length !== 3 || mapped.some((item) => !Number.isInteger(item.routeOrder) || item.routeDistanceKm !== optimized.body.distanceKm) || saved.jobs.some((item) => item.id.startsWith('route-conflict-') && item.routeOrder)) throw new Error('route optimization did not safely order mapped stops, reject infeasible windows, report distance, or deduplicate');
