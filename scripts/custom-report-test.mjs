...
try {
  ...
  const view = await request('/api/reports/custom/views', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': 'custom-report-view-test' }, body: JSON.stringify({ name: 'Cash and margin month', metrics: ['Cash collected', 'Gross margin'], startDate: '2026-01-01', endDate: '2026-01-31' }) });
  const viewDuplicate = await request('/api/reports/custom/views', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': 'custom-report-view-test' }, body: JSON.stringify({ name: 'Cash and margin month', metrics: ['Cash collected', 'Gross margin'], startDate: '2026-01-01', endDate: '2026-01-31' }) });
