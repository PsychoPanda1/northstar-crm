...
try {
  ...
  const catalogResponse = await fetch(`${base}/api/catalog`, { method: 'POST', headers: { authorization: `Bearer ${login.token}`, 'content-type': 'application/json', 'idempotency-key': 'plan-visit-catalog' }, body: JSON.stringify({ name: 'Annual plumbing care', description: 'Recurring care with required field documentation', priceFrom: '$199', durationMinutes: 60, formNames: ['Maintenance inspection', 'Customer handoff'] }) }); const catalog = await catalogResponse.json(); assert(catalogResponse.status === 201 && catalog.id, 'plan catalog creation failed');
  const planResponse = await fetch(`${base}/api/plans`, { method: 'POST', headers: { authorization: `Bearer ${login.token}`, 'content-type': 'application/json', 'idempotency-key': 'plan-visit-plan' }, body: JSON.stringify({ customerId: customer.id, service: 'Annual plumbing care', catalogItemId: catalog.id, amount: 199, renewal: 'Annual', visitsIncluded: 1 }) }); const planBody = await planResponse.json(); assert(planResponse.status === 201 && planBody.id && planBody.catalogItemId === catalog.id, 'plan creation failed');
  ...
  const response = await schedule(); const body = await response.json(); const duplicate = await schedule(); const duplicateBody = await duplicate.json(); const exhausted = await fetch(`${base}/api/public/customer-portal/service-plan-visit?token=${encodeURIComponent(booking.customerPortalAccessToken)}`, { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'plan-visit-after-allowance' }, body: JSON.stringify({ planId: planBody.id, slotId: nextAvailability.slotOptions?.find((slot) => slot.id !== nextSlot.id)?.id || nextSlot.id }) }); const exhaustedBody = await exhausted.json();
  ...
  assert(response.status === 201 && body.job?.planId === planBody.id && body.job?.requiredForms?.length === 2 && body.job.requiredForms[0].formName === 'Maintenance inspection' && body.notification?.template === 'confirmation' && duplicate.status === 200 && duplicateBody.duplicate && duplicateBody.id === body.id && exhausted.status === 409 && exhaustedBody.error === 'service_plan_visit_allowance_exhausted' && portal.plans?.some((item) => item.id === planBody.id && item.visitsScheduled === 1 && item.visitsAvailableToSchedule === 0 && item.nextVisit?.startsAt === nextSlot.startsAt), 'customer plan visit workflow failed');
