...
const waitForServer = async () => { for (let attempt = 0; attempt < 200; attempt += 1) { try { if ((await fetch(`${base}/api/health`)).ok) return; } catch {} await new Promise((resolve) => setTimeout(resolve, 50)); } throw new Error('job import server did not start'); };
  const rows = [{ externalId: 'legacy-job-1', customerPhone: '(843) 555-0177', service: 'Drain cleaning', time: '2026-08-14 09:00 AM', status: 'Completed', priority: 'Normal', technician: 'Alex Rivera', completionNote: 'Imported historical closeout.' }, { externalId: 'legacy-job-2', customerName: 'Missing Customer', service: 'Unknown service', time: '2026-08-15 10:00 AM', status: 'Completed' }, { externalId: 'legacy-job-3', customerId: 'import-customer', service: 'Electrical inspection', time: '2026-08-16 11:00 AM', timeZone: 'Not/A-Timezone' }];
try {
  await waitForServer();
