...
const env = { ...process.env, NODE_ENV: 'development', PORT: String(port), NORTHSTAR_DATA_FILE: dataFile, NORTHSTAR_SESSION_FILE: `${dataFile}.sessions`, NORTHSTAR_DOCUMENT_PROVIDER_URL: `http://127.0.0.1:${providerPort + 1}`, NORTHSTAR_TENANT_PROVIDER_CONFIG_JSON: JSON.stringify({ [tenantId]: { document: { url: `http://127.0.0.1:${providerPort}`, apiKey: 'tenant-document-key' } } }) };
...
try {
  ...
  if (!login.response.ok || result.response.status !== 200 || result.body.delivered !== 1 || received.length !== 1 || received[0].headers['idempotency-key'] !== delivery.id || received[0].headers.authorization !== 'Bearer tenant-document-key' || received[0].body.documentType !== 'estimate' || received[0].body.documentId !== 'EST-document-1' || delivery.status !== 'Delivered' || !delivery.providerReference || retryDelivered.response.status !== 409 || listed.response.status !== 200 || listed.body.deliveries?.[0]?.id !== delivery.id || health.response.status !== 200 || health.body.documents?.delivered !== 1 || health.body.documents?.retrying !== 0 || health.body.checks?.documentProvider !== true || !String(received[0].body.pdfUrl).includes('/api/public/estimate/pdf')) throw new Error('tenant document provider handoff, delivery health, ledger, auth, or retry guard failed');
