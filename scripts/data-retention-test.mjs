...
try {
  ...
  const configured = await request('/api/reports/data-retention', { headers: ownerHeaders }); const manifest = await request('/api/public/tenant?service=plumbing');
  ...
  if (initial.response.status !== 200 || initial.body.policy?.mode !== 'review_only' || !initialActivities?.candidateIds?.includes('retention-old-1') || update.response.status !== 200 || update.body.workspace?.dataRetentionDays !== 30 || configured.body.policy?.retentionDays !== 30 || dispatcher.response.status !== 403 || manifest.body.tenant?.dataRetentionDays !== undefined) throw new Error('data retention contract failed');
