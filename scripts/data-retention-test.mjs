...
try {
  ...
  if (initial.response.status !== 200 || initial.body.policy?.mode !== 'review_only' || !initialActivities?.candidateIds?.includes('retention-old-1') || update.response.status !== 200 || update.body.workspace?.dataRetentionDays !== 30 || configured.body.policy?.retentionDays !== 30 || dispatcher.response.status !== 403 || manifest.body.tenant?.dataRetentionDays !== undefined || archive.response.status !== 200 || !archiveId || archivedReport.body.candidateRecords !== 0 || archivedReport.body.archivedRecords !== 1 || archivedReport.body.archivedEntries?.[0]?.archiveId !== archiveId || Object.prototype.hasOwnProperty.call(archivedReport.body.archivedEntries?.[0] || {}, 'record') || restore.response.status !== 200 || restoredReport.body.candidateRecords !== 1 || restoredReport.body.archivedRecords !== 0 || (restoredReport.body.archivedEntries || []).length !== 0) throw new Error('data retention contract failed');
