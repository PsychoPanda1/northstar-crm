...
try {
  ...
  const overdueRequest = (before.body.items || []).find((item) => item.requestId === 'sla-request-1');
  const freshRequest = (before.body.items || []).find((item) => item.requestId === 'sla-request-fresh');
  const urgentRequest = (before.body.items || []).find((item) => item.requestId === 'sla-request-urgent');
