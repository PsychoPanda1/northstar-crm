...
try {
  ...
  const bulkAssignmentKey = 'sla-request-bulk-assignment-1';
  const bulkAssignment = await request('/api/requests/bulk-assign', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': bulkAssignmentKey }, body: JSON.stringify({ requestIds: ['REQ-sla-request-1', 'REQ-sla-request-urgent'], assignedTo: 'Taylor Brooks' }) });
  const bulkAssignmentDuplicate = await request('/api/requests/bulk-assign', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': bulkAssignmentKey }, body: JSON.stringify({ requestIds: ['REQ-sla-request-1', 'REQ-sla-request-urgent'], assignedTo: 'Taylor Brooks' }) });
  const bulkAssignmentConflict = await request('/api/requests/bulk-assign', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': bulkAssignmentKey }, body: JSON.stringify({ requestIds: ['REQ-sla-request-1', 'REQ-sla-request-urgent'], assignedTo: 'Jordan Smith' }) });
  const bulkPriorityKey = 'sla-request-bulk-priority-1';
  const bulkPriority = await request('/api/requests/bulk-priority', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': bulkPriorityKey }, body: JSON.stringify({ requestIds: ['REQ-sla-request-1', 'REQ-sla-request-urgent'], priority: 'Urgent' }) });
  const bulkPriorityDuplicate = await request('/api/requests/bulk-priority', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': bulkPriorityKey }, body: JSON.stringify({ requestIds: ['REQ-sla-request-1', 'REQ-sla-request-urgent'], priority: 'Urgent' }) });
  const bulkPriorityConflict = await request('/api/requests/bulk-priority', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': bulkPriorityKey }, body: JSON.stringify({ requestIds: ['REQ-sla-request-1', 'REQ-sla-request-urgent'], priority: 'High' }) });
  ...
  const urgentQueue = await request('/api/requests?priority=Urgent', { headers });
  const assignedQueue = await request('/api/requests?assignedTo=Taylor%20Brooks', { headers });
  ...
  const bulkStatusKey = 'sla-request-bulk-status-1';
  const bulkStatus = await request('/api/requests/bulk-status', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': bulkStatusKey }, body: JSON.stringify({ requestIds: ['REQ-sla-request-1', 'REQ-sla-request-urgent'], status: 'In progress' }) });
  const bulkStatusDuplicate = await request('/api/requests/bulk-status', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': bulkStatusKey }, body: JSON.stringify({ requestIds: ['REQ-sla-request-1', 'REQ-sla-request-urgent'], status: 'In progress' }) });
  const bulkStatusConflict = await request('/api/requests/bulk-status', { method: 'POST', headers: { ...headers, 'content-type': 'application/json', 'idempotency-key': bulkStatusKey }, body: JSON.stringify({ requestIds: ['REQ-sla-request-fresh', 'REQ-sla-request-urgent'], status: 'In progress' }) });
  const requestQueue = await request('/api/requests', { headers });
  const requestQueueIds = (requestQueue.body.items || []).map((item) => item.id);
