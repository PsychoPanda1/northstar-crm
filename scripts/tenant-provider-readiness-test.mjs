...
const initialState = JSON.stringify({ 'johnson-service-co': {}, 'clearwater-plumbing': {}, 'lowcountry-wash-co': {}, 'palmetto-electric': {}, 'harbor-shine': {} });
writeFileSync(dataFile, initialState); writeFileSync(`${dataFile}.backup`, initialState);
...
try {
  ...
  const paymentSetupHealth = login.body.token ? await request('/api/integrations/payment-setup/health', { headers: { authorization: `Bearer ${login.body.token}` } }) : null;
