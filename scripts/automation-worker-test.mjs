...
const env = { ...process.env, NODE_ENV: 'development', PORT: String(port), NORTHSTAR_DATA_FILE: dataFile, NORTHSTAR_SESSION_FILE: sessionFile, NORTHSTAR_AUTOMATION_INTERVAL_MINUTES: '15', NORTHSTAR_MESSAGE_PROVIDER_URL: `http://127.0.0.1:${providerPort}`, NORTHSTAR_PAYMENT_PROVIDER_URL: `http://127.0.0.1:${providerPort}` };
let child;
...
try {
  ...
  if (!messages.response.ok || !messages.body.items?.some((item) => item.jobId === 'automation_job' && item.template === 'appointment_reminder' && item.status === 'Sent') || !audit.response.ok || !audit.body.items?.some((item) => item.action === 'automation.scheduled') || persisted.paymentIntents?.find((item) => item.id === 'automation_intent')?.status !== 'Submitted (provider pending)' || providerCalls < 2) throw new Error('scheduled automation did not queue and dispatch reminders and payment intents');
