...
try {
  ...
  if (!messages.response.ok || messages.body.items?.some((item) => item.jobId === 'automation_job' && item.template === 'appointment_reminder') || messages.body.items?.some((item) => item.planId === 'automation_plan' && item.template === 'plan_renewal_reminder') || !audit.response.ok || !audit.body.items?.some((item) => item.action === 'automation.scheduled') || !persisted.invoices?.some((item) => item.planId === 'automation_plan' && item.status === 'Due') || !persisted.planBillingCycles?.some((item) => item.trigger === 'scheduled') || persisted.paymentIntents?.find((item) => item.id === 'automation_intent')?.status !== 'Submitted (provider pending)' || providerCalls < 1) throw new Error('scheduled automation did not bill plans, honor opt-outs, suppress messages, and dispatch payment intents');
