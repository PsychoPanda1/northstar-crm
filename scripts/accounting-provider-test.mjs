...
try {
  ...
  if (preflightHealth.body.accounting?.pending !== 4 || dispatch.response.status !== 200 || dispatch.body.delivered !== 3 || dispatch.body.retrying !== 1 || retry.response.status !== 200 || retry.body.sync?.syncState !== 'Pending' || retryDispatch.body.delivered !== 1 || refund.response.status !== 201 || updateDispatch.body.delivered !== 2 || duplicate.body.attempted !== 0 || received.length !== 6 || received.some((item) => item.headers.authorization !== 'Bearer tenant-accounting-key') || !keys.includes('invoice:invoice-erp-1') || !keys.includes('payment:payment-erp-1') || !keys.includes('purchase_order:po-erp-1') || !keys.includes('vendor_bill:vb-erp-1') || sync.some((item) => item.syncState !== 'Delivered') || health.body.checks?.accountingProvider !== true || health.body.accounting?.pending !== 0) throw new Error('accounting provider delivery or health contract failed');
