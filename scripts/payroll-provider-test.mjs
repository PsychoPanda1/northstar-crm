...
const received = [];
const provider = createServer((req, res) => { let raw = ''; req.on('data', (chunk) => { raw += chunk; }); req.on('end', () => { received.push({ headers: req.headers, body: JSON.parse(raw || '{}') }); res.writeHead(200, { 'content-type': 'application/json' }); res.end(JSON.stringify({ id: `payroll-${received.length}`, status: 'accepted' })); }); });
...
const env = { ...process.env, NODE_ENV: 'development', NORTHSTAR_PAYROLL_PROVIDER_URL: `http://127.0.0.1:${providerPort}/payroll`, NORTHSTAR_PAYROLL_PROVIDER_API_KEY: 'payroll-test-key', PORT: String(port), NORTHSTAR_DATA_FILE: dataFile, NORTHSTAR_SESSION_FILE: `${dataFile}.sessions` };
...
try {
  ...
  if (dispatch.response.status !== 200 || dispatch.body.delivered !== 1 || duplicate.body.attempted !== 0 || received.length !== 1 || received[0].headers.authorization !== 'Bearer payroll-test-key' || received[0].headers['idempotency-key'] !== 'PAY-provider-1' || run.providerSyncState !== 'Delivered' || health.body.checks?.payrollProvider !== true || health.body.payroll?.pending !== 0) throw new Error('payroll provider delivery or health contract failed');
