...
const env = { ...process.env, NODE_ENV: 'development', NORTHSTAR_PAYROLL_PROVIDER_URL: `http://127.0.0.1:${providerPort}/payroll`, NORTHSTAR_PAYROLL_PROVIDER_API_KEY: 'payroll-test-key', NORTHSTAR_TENANT_PROVIDER_CONFIG_JSON: JSON.stringify({ [tenantId]: { payroll: { url: `http://127.0.0.1:${providerPort}/tenant-payroll`, apiKey: 'tenant-payroll-key' } } }), NORTHSTAR_PAYROLL_RETRY_LIMIT: '1', PORT: String(port), NORTHSTAR_DATA_FILE: dataFile, NORTHSTAR_SESSION_FILE: `${dataFile}.sessions` };
...
try {
  ...
  if (dispatch.response.status !== 200 || dispatch.body.retrying !== 1 || retry.response.status !== 200 || retryDispatch.body.delivered !== 1 || duplicate.body.attempted !== 0 || received.length !== 1 || received[0].headers.authorization !== 'Bearer tenant-payroll-key' || received[0].headers['idempotency-key'] !== 'PAY-provider-1' || run.providerSyncState !== 'Delivered' || health.body.checks?.payrollProvider !== true || health.body.payroll?.pending !== 0) throw new Error('payroll provider delivery or health contract failed');
