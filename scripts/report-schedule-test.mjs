...
try {
  ...
  const updateHeaders = { ...headers, 'content-type': 'application/json', 'idempotency-key': 'report-schedule-update-test' };
  const updated = await request(`/api/reports/custom/schedules/${encodeURIComponent(created.body.schedule?.id || '')}`, { method: 'PATCH', headers: updateHeaders, body: JSON.stringify({ enabled: false, frequency: 'daily' }) });
  const updatedDuplicate = await request(`/api/reports/custom/schedules/${encodeURIComponent(created.body.schedule?.id || '')}`, { method: 'PATCH', headers: updateHeaders, body: JSON.stringify({ enabled: false, frequency: 'daily' }) });
  const updatedConflict = await request(`/api/reports/custom/schedules/${encodeURIComponent(created.body.schedule?.id || '')}`, { method: 'PATCH', headers: updateHeaders, body: JSON.stringify({ enabled: true, frequency: 'daily' }) });
  if (!login.response.ok || schedules.response.status !== 200 || schedules.body.schedules?.[0]?.id !== 'RS-DUE-1' || messages.response.status !== 200 || !messages.body.items?.some((item) => item.reportScheduleId === 'RS-DUE-1' && item.template === 'scheduled_report' && item.to === 'owner@example.com' && item.reportCsv?.includes('"group","metric","value","detail"')) || created.response.status !== 201 || duplicate.response.status !== 200 || duplicate.body.duplicate !== true || invalid.response.status !== 422 || updated.response.status !== 200 || updated.body.schedule?.enabled !== false || updatedDuplicate.response.status !== 200 || updatedDuplicate.body.duplicate !== true || updatedConflict.response.status !== 409) throw new Error('scheduled report creation, idempotency, validation, automation queue, or update failed');
