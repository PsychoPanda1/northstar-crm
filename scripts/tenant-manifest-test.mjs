...
try {
  ...
  assert(response.status === 200 && body.tenant?.contactPhone === tenant.contactPhone && body.tenant?.contactEmail === tenant.contactEmail && body.tenant?.serviceArea === tenant.serviceArea && JSON.stringify(body.tenant?.leadStages) === JSON.stringify(tenant.leadStages) && technicianEndpointKeys.every((key) => typeof body.integration?.technicianEndpoints?.[key] === 'string') && body.integration?.technicianEndpoints?.job === '/api/public/technician-job' && body.integration?.technicianEndpoints?.complete === '/api/public/technician-job/complete' && body.integration?.technicianEndpoints?.paymentIntent === '/api/public/technician-job/payment-intent' && body.integration?.capabilities?.technicianFieldOperations === true && body.integration?.capabilities?.technicianOfflineReplay === true, 'public tenant configuration was not exposed safely');
