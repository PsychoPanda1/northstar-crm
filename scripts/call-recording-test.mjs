...
try {
  ...
  if (!created.response.ok || !call?.recordingAvailable || Object.prototype.hasOwnProperty.call(call, 'recordingUrl') || recording.response.status !== 200 || recording.body.recording?.url !== 'https://recordings.example.test/call/abc' || revoked.response.status !== 200 || revoked.body.revoked !== true || revokedAgain.response.status !== 200 || revokedAgain.body.duplicate !== true || afterRevoke.response.status !== 404 || invalid.response.status !== 422 || expiredCreated.response.status !== 201 || expiredRecording.response.status !== 410 || accountantRecording.response.status !== 403) throw new Error('call recording metadata, protected access, URL validation, expiry, revocation, or list redaction failed');
