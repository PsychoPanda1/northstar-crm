...
try {
  ...
  if (!login.response.ok || detail.response.status !== 200 || detail.body.estimate?.lineItems?.[1]?.description !== 'Installation labor' || result.response.status !== 201 || invoice?.estimateId !== estimate.id || invoice.amount !== estimate.amount || invoice.lineItems?.length !== estimate.lineItems.length || invoice.lineItems?.[1]?.description !== 'Installation labor' || invoice.subtotal !== estimate.subtotal || invoice.discount !== estimate.discount || bulk.response.status !== 201 || bulkInvoice?.estimateId !== bulkEstimate.id || bulkInvoice.amount !== bulkEstimate.amount || bulkInvoice.lineItems?.[0]?.description !== 'Maintenance visit' || bulkInvoice.taxRate !== bulkEstimate.taxRate) throw new Error('accepted estimate pricing did not carry into completed job invoice');
