...
try {
  ...
  assert(first.response.status === 200 && first.body.jobs?.length === 2 && first.body.jobs[0].slotId === changes[0].slotId && duplicate.response.status === 200 && duplicate.body.duplicate === true && restartedLogin.response.ok && restartedDuplicate.response.status === 200 && restartedDuplicate.body.duplicate === true && conflict.response.status === 422 && conflict.body.error === 'valid_bulk_reschedule_required' && messages.body.items?.filter((item) => item.template === 'rescheduled' && jobIds.includes(item.jobId)).length === 2, 'bulk reschedule did not apply, deduplicate across restart, reject duplicate slots, or notify customers safely');
