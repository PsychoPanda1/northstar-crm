...
try {
  ...
  const restartedLogin = await post('/api/auth/demo-login?service=plumbing', { service: 'plumbing', role: 'owner' });
  const restartedDuplicate = await post('/api/dispatch/bulk-reschedule', { changes }, { authorization: `Bearer ${restartedLogin.body.token}`, 'idempotency-key': 'bulk-reschedule-1' });
  const changedRetry = await post('/api/dispatch/bulk-reschedule', { changes: [{ jobId: jobIds[0], slotId: availability.body.slotOptions[2].id }, { jobId: jobIds[1], slotId: availability.body.slotOptions[3].id }] }, { authorization: `Bearer ${restartedLogin.body.token}`, 'idempotency-key': 'bulk-reschedule-1' });
  const technicianLogin = await post('/api/auth/demo-login?service=plumbing', { service: 'plumbing', role: 'technician' });
  const technicianAttempt = await post('/api/dispatch/bulk-reschedule', { changes }, { authorization: `Bearer ${technicianLogin.body.token}`, 'idempotency-key': 'bulk-reschedule-technician' });
  const capacityDate = new Intl.DateTimeFormat('en-CA', { timeZone: availability.body.timeZone || 'America/New_York' }).format(new Date(availability.body.slotOptions[0].startsAt));
  const capacity = await post('/api/dispatch/capacity', { date: capacityDate, technician: 'Alex Rivera', targetMinutes: 60 }, headers);
  const capacityAttempt = await post('/api/dispatch/bulk-reschedule', { changes }, { ...headers, 'idempotency-key': 'bulk-reschedule-capacity' });
  const capacityDuplicate = await post('/api/dispatch/bulk-reschedule', { changes }, { ...headers, 'idempotency-key': 'bulk-reschedule-1' });
  ...
  assert(first.response.status === 200 && first.body.jobs?.length === 2 && first.body.jobs[0].slotId === changes[0].slotId && duplicate.response.status === 200 && duplicate.body.duplicate === true && restartedLogin.response.ok && restartedDuplicate.response.status === 200 && restartedDuplicate.body.duplicate === true && changedRetry.response.status === 409 && changedRetry.body.error === 'idempotency_key_reused' && technicianLogin.response.ok && technicianAttempt.response.status === 403 && capacity.response.status === 200 && capacityAttempt.response.status === 409 && capacityAttempt.body.error === 'capacity_target_conflict' && capacityDuplicate.response.status === 200 && capacityDuplicate.body.duplicate === true && conflict.response.status === 422 && conflict.body.error === 'valid_bulk_reschedule_required' && messages.body.items?.filter((item) => item.template === 'rescheduled' && jobIds.includes(item.jobId)).length === 2, 'bulk reschedule did not apply, enforce restart-safe idempotency, role boundaries, and capacity targets, reject duplicate slots, or notify customers safely');
