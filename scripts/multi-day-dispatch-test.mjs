...
try {
  ...
  const calendarResponse = await fetch(`${base}/api/dispatch/route-calendar?date=2026-09-08`, { headers });
  const calendar = await calendarResponse.text();
  const ids = middle.body.items?.map((item) => item.id) || [];
  if (!login.response.ok || middle.response.status !== 200 || last.response.status !== 200 || summary.response.status !== 200 || summary.body.totalStops !== 2 || calendarResponse.status !== 200 || !calendar.includes('JOB-MULTI-DAY@northstar-crm') || !ids.includes('JOB-MULTI-DAY') || !ids.includes('JOB-SINGLE-DAY') || !last.body.items?.some((item) => item.id === 'JOB-MULTI-DAY')) throw new Error('multi-day jobs were not projected across every covered dispatch date');
