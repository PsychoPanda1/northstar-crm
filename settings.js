(() => {
  ...
  const openSettings = async (event) => {
    ...
    try {
      ...
      list.innerHTML = `<div class="report-period">Authenticated tenant configuration · secrets are never displayed</div><article class="report-card"><div><span class="record-id">WORKSPACE</span><h3>${escapeHtml(workspace.businessName || 'Configured workspace')}</h3><p>${escapeHtml(workspace.serviceLabel || 'Service operations')} · ${escapeHtml(workspace.timeZone || 'Timezone configured server-side')}</p></div></article><article class="record-card"><div><span class="record-id">DATA PORTABILITY</span><h3>Owner migration snapshot</h3><p>Download a tenant-scoped JSON snapshot for backup or managed-storage migration. Sensitive control fields are removed.</p><div class="record-actions"><button class="ghost-btn" type="button" data-settings-snapshot>Download snapshot</button></div></div></article><article class="record-card"><div><span class="record-id">LANDING PAGE HANDOFF</span><h3>Connected service: ${escapeHtml(serviceKey || 'current tenant')}</h3><p>Booking <code>${escapeHtml(origin)}/booking.html?service=${encodeURIComponent(serviceKey)}</code> · Owner portal <code>${escapeHtml(origin)}/portal?service=${encodeURIComponent(serviceKey)}</code></p></div></article><article class="record-card"><div><span class="record-id">PUBLIC INTEGRATION</span><h3>Versioned tenant manifest</h3><p><code>/api/public/tenant?service=${encodeURIComponent(serviceKey)}</code> · availability, catalog, leads, bookings, and owner handoff are tenant-scoped.</p></div></article>${checks}`;
      const snapshotButton = list.querySelector('[data-settings-snapshot]');
      if (snapshotButton && repository.session?.owner?.role === 'owner') snapshotButton.addEventListener('click', async () => { snapshotButton.disabled = true; try { await repository.exportRecords('tenant-snapshot'); } catch { snapshotButton.disabled = false; return; } snapshotButton.textContent = 'Snapshot downloaded'; });
      if (snapshotButton && repository.session?.owner?.role !== 'owner') snapshotButton.remove();
    } catch {
