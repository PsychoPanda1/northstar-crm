(() => {
  ...
  const renderFields = async () => {
    ...
    if (action === 'revoke') { fields.innerHTML = '<div class="empty-state">Loading pending invites…</div>'; try { const invites = (await repository.listUserInvites()).items || []; const pending = invites.filter((item) => item.status === 'Pending'); fields.innerHTML = pending.length ? field('Pending invite', `<select name="inviteId">${pending.map((invite) => `<option value="${escapeHtml(invite.id)}">${escapeHtml(invite.name)} · ${escapeHtml(invite.email)}</option>`).join('')}</select>`) : '<div class="empty-state">No pending invitations to revoke.</div>'; } catch { fields.innerHTML = '<div class="empty-state">Invitations are unavailable.</div>'; } }
  };
