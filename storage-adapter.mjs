...
export const createSqliteStore = (DatabaseSync, file) => {
  ...
  const backupTo = (destination) => { const temporary = `${destination}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`; database.exec(`VACUUM INTO '${temporary.replace(/'/g, "''")}'`); try { if (existsSync(destination)) unlinkSync(destination); } catch {} try { renameSync(temporary, destination); } catch (error) { try { if (existsSync(temporary)) unlinkSync(temporary); } catch {} throw error; } };
