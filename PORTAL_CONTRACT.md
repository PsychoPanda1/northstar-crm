...
- `GET /api/public/availability?service=...&days=...` and `POST /api/public/bookings?service=...` → expose up to 14 days of weekday capacity (with legacy relative labels still supported), date-specific slot IDs, and UTC appointment timestamps in the tenant IANA timezone; active jobs and configured aggregate daily capacity targets remove unavailable slots before booking creates an idempotent tenant-routed customer/job booking and a system-attributed `booking.received` audit event
...
- Assignment, rescheduling, and visit creation reject active same-technician/time conflicts across both parent appointments and visit-level schedules with `409 technician_schedule_conflict`
...
- `POST /api/public/customer-portal/reschedule?token=...` → move a customer-owned, active or no-show appointment to one of its returned `availableRescheduleSlots`; no-show recovery returns the job to `Confirmed` when a technician is retained or `Unassigned` otherwise, and the mutation remains customer-token-only, idempotent, conflict-checked, and auditable
...
- `POST /api/jobs/:id/rebook` → owner/dispatcher-only recovery of a canceled or no-show appointment into an available, capacity-aware slot; technician conflicts are checked, the transition is audited, customer confirmation is queued when applicable, and same-key retries return the original result
- `GET /api/notifications` → owner action items include customer cancellation notifications with a stable `jobId`; unread items can be acknowledged through `POST /api/notifications/:id/read`, and the dispatch UI exposes direct rebooking
