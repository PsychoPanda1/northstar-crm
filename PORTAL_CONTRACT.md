...
- `POST /api/jobs/:id/rebook` → owner/dispatcher-only recovery of a canceled appointment into an available, capacity-aware slot; technician conflicts are checked, the transition is audited, customer confirmation is queued when applicable, and same-key retries return the original result
