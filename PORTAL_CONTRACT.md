...
- `POST /api/dispatch/route-order` → owner/dispatcher-only manual ordering of a technician's same-day normalized stops; job IDs must be unique and match the requested tenant-local date and technician, and retries support idempotency.
