...
- `POST /api/public/customer-portal/preferences?token=...` → customer-token-only update of SMS/email opt-out flags with idempotent replay and customer-attributed audit history; preference fields are not included in the portal GET projection
