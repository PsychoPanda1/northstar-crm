...
- `GET /api/public/availability?service=...` and `POST /api/public/bookings?service=...` → expose date-specific slot IDs plus UTC appointment timestamps in the tenant IANA timezone and create an idempotent tenant-routed customer/job booking; legacy relative labels remain supported while production capacity is connected
...
- `POST /api/public/customer-portal/request?token=...` accepts an optional `Idempotency-Key` and returns `duplicate: true` for safe retries of the same customer request
...
- Report metrics also summarize estimate close rate, memberships sold, no-shows, tracked field hours, material spend, logged labor cost, and gross margin from recorded job data
...
- `GET /api/purchase-orders`, `POST /api/purchase-orders`, and `POST /api/purchase-orders/:id/receive` → create and receive replenishment orders with automatic stock reconciliation; receipt accepts an optional positive whole-number `quantity`, tracks `receivedQuantity`, supports split shipments, and records an owner-auditable receipt event
