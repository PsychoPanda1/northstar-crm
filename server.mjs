...
const recordsFor = (tenantId, type, search = '') => {
  ...
  const all = { customers, leads, estimates, invoices, plans, activities, dispatch, team }[type] || [];
...
const server = createServer(async (req, res) => {
  try {
    ...
    const recordsMatch = pathname.match(/^\/api\/(customers|leads|estimates|invoices|plans|activities|dispatch|team)$/);
