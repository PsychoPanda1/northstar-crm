...
const blankState = () => ({ completedTasks: [], lastAction: null, leads: [], jobs: [], estimates: [], invoices: [], plans: [] });
...
const recordsFor = (tenantId, type, search = '') => {
  ...
  const all = { customers, leads, estimates, invoices, plans, dispatch }[type] || [];
...
const server = createServer(async (req, res) => {
  try {
    ...
    const recordsMatch = pathname.match(/^\/api\/(customers|leads|estimates|invoices|plans|dispatch)$/);
