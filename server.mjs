...
const blankState = () => ({ completedTasks: [], lastAction: null, leads: [], jobs: [], estimates: [], invoices: [] });
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (pathname === '/api/session' && req.method === 'GET') { const claims = authenticate(req); if (!claims) return json(res, 401, { error: 'unauthorized' }); return json(res, 200, { owner: { id: claims.sub, name: owners.jordan.name, role: owners.jordan.role }, tenant: tenants[claims.tenantId], permissions: ['dashboard:read', 'tasks:write', 'actions:write'] }); }
    ...
    const recordsMatch = pathname.match(/^\/api\/(customers|leads|estimates|invoices|dispatch)$/);
