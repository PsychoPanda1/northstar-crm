...
try {
  ...
  assert(createdCustomer.response.status === 201 && directJob.response.status === 201 && conflictingDirectJob.response.status === 409 && conflictingDirectJob.body.error === 'appointment_time_unavailable' && missingCustomerJob.response.status === 404, 'direct job scheduling conflict protection failed');
