...
try {
  ...
  assert(converted.response.status === 201 && materialUse.response.ok && materialUse.body.job.materials[0].quantity === 3 && labor.response.status === 201 && labor.body.jobCost.laborCost === '$150.00' && jobCosts.body.items.some((item) => item.jobId === converted.body.job.id && item.totalCost === '$175.50' && item.grossMargin === '$-175.50') && lowStockMaterials.body.items[0].onHand === 1 && lowStockMaterials.body.items[0].status === 'Low stock' && inventoryLedger.response.ok && inventoryLedger.body.items.some((item) => item.material === '1/2 inch copper coupling' && item.type === 'Consumption' && item.quantity === -3) && inventoryLedger.body.items.some((item) => item.material === '1/2 inch copper coupling' && item.type === 'Receipt' && item.quantity === 4) && purchaseOrder.response.status === 201 && openPurchaseOrders.body.items[0].status === 'Open' && receivedPurchaseOrder.response.ok && receivedPurchaseOrder.body.material.onHand === 7 && converted.body.job.leadId === leadList.body.items[0].id && converted.body.job.customerId === converted.body.customer.id && convertedCustomer.body.items.length === 1 && publicStatus.body.status === 'Unassigned' && publicStatus.body.appointment.label === publicStatus.body.time && publicStatus.body.appointment.slotId === null, 'lead conversion, inventory, or job costing failed');
