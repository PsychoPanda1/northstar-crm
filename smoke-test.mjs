...
try {
  ...
  assert(customerLink.response.ok && customerPortal.body.customer.name === 'Smoke Lead' && customerPortal.body.locations.length === 2 && customerPortal.body.locations.some((item) => item.address.includes('200 Meeting')) && customerPortal.body.jobs.length >= 2 && customerPortal.body.assets.length === 1 && !customerPortal.body.customer.tenantId && customerPortalWithEstimate.body.estimates.some((item) => item.id === portalEstimate.body.id && item.estimateApprovalToken) && portalApproval.body.status === 'Accepted' && estimateJob.response.status === 201 && estimateJob.body.job.estimateId === portalEstimate.body.id && estimateJob.body.job.checklist.length === 3, 'customer portal workflow failed');
