...
const staffDigest = createHmac('sha256', 'northstar-local-demo-secret-change-me').update(staffPassword).digest('hex');
const serverEnv = { ...process.env, NODE_ENV: 'production', PORT: String(port), NORTHSTAR_DATA_FILE: dataFile, NORTHSTAR_PAYMENT_WEBHOOK_SECRET: webhookSecret, NORTHSTAR_MESSAGE_WEBHOOK_SECRET: messageWebhookSecret, NORTHSTAR_ALLOWED_ORIGINS: 'https://plumbing.example', NORTHSTAR_OWNER_EMAIL: ownerEmail, NORTHSTAR_OWNER_PASSWORD_DIGEST: ownerDigest, NORTHSTAR_STAFF_JSON: JSON.stringify([{ id: 'staff_configured_dispatcher', name: 'Configured Dispatcher', email: 'dispatcher@example.test', passwordDigest: staffDigest, role: 'dispatcher', tenantId: 'clearwater-plumbing' }]) };
...
try {
  ...
  const partialPurchaseOrder = await request(`/api/purchase-orders/${purchaseOrder.body.id}/receive`, jsonOptions('POST', { quantity: 2 }, token));
  ...
  assert(converted.response.status === 201 && converted.body.customer.phone === '843-555-0100' && converted.body.customer.email === 'smoke.lead@example.test' && materialUse.response.ok && materialUse.body.job.materials[0].quantity === 3 && labor.response.status === 201 && labor.body.jobCost.laborCost === '$150.00' && jobCosts.body.items.some((item) => item.jobId === converted.body.job.id && item.totalCost === '$175.50' && item.grossMargin === '$-175.50') && lowStockMaterials.body.items[0].onHand === 1 && lowStockMaterials.body.items[0].status === 'Low stock' && inventoryLedger.response.ok && inventoryLedger.body.items.some((item) => item.material === '1/2 inch copper coupling' && item.type === 'Consumption' && item.quantity === -3) && inventoryLedger.body.items.some((item) => item.material === '1/2 inch copper coupling' && item.type === 'Receipt' && item.quantity === 4) && purchaseOrder.response.status === 201 && openPurchaseOrders.body.items[0].status === 'Open' && partialPurchaseOrder.response.ok && partialPurchaseOrder.body.order.status === 'Partially received' && partialPurchaseOrder.body.order.receivedQuantity === 2 && partialPurchaseOrder.body.material.onHand === 3 && receivedPurchaseOrder.response.ok && receivedPurchaseOrder.body.order.status === 'Received' && receivedPurchaseOrder.body.order.receivedQuantity === 6 && receivedPurchaseOrder.body.material.onHand === 7 && inventoryAudit.response.ok && inventoryAudit.body.items.filter((item) => item.entityId === purchaseOrder.body.id).length === 2 && conversionAudit.response.ok && conversionAudit.body.items.some((item) => item.entityId === leadList.body.items[0].id && item.action === 'lead.converted') && converted.body.job.leadId === leadList.body.items[0].id && converted.body.job.customerId === converted.body.customer.id && convertedCustomer.body.items.length === 1 && publicStatus.body.status === 'Unassigned' && publicStatus.body.appointment.label === publicStatus.body.time && publicStatus.body.appointment.slotId === null, 'lead conversion, inventory, or job costing failed');
  ...
  const asset = await request('/api/assets', jsonOptions('POST', { customer: 'Smoke Lead', name: 'Tankless water heater', serial: 'TS-42', installed: '2026-09-04', warrantyThrough: warrantySoon }, token));
  ...
  assert(asset.response.status === 201 && asset.body.warrantyThrough === warrantySoon && assets.body.items.length === 1 && assets.body.items[0].customer === 'Smoke Lead' && (await assetExport.text()).includes('TS-42') && assetAudit.body.items.some((item) => item.action === 'asset.created') && warrantyNotifications.body.items.some((item) => item.title === 'Warranty expiring soon'), 'customer asset workflow failed');
  ...
  const pricedInvoice = await request('/api/invoices', jsonOptions('POST', { estimateId: pricedEstimate.body.id }, token));
