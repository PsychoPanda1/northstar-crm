...
The tenant discovery response includes `integration.version: 2`, copyable `bookingPath`, `ownerPortalPath`, `ownerAuthEndpoint`, `ownerAuthRefreshEndpoint`, `ownerAuthLogoutEndpoint`, `ownerAuthMethods`, `ownerOidcAuthEndpoint`, `leadEndpoint`, `bookingEndpoint`, and `availabilityEndpoint` values, the complete `technicianEndpoints` map, complete `customerPortalEndpoints` and `estimateEndpoints` maps, the `reviewEndpoint`, capability flags, and `bookingRequirements`. `ownerAuthMethods` always includes `password`; it also includes `oidc` only when that tenant has a provisioned, verified OIDC owner account. The OIDC endpoint exchanges a signed identity-provider ID token for the same tenant-scoped session contract and must receive the manifest service key in both the request context and payload; the server rejects an identity provisioned for another service tenant. The refresh endpoint likewise requires the service context and rejects a token used from a different mapped landing page. Logout requires the current authenticated session token; it revokes it server-side and clears the HTTP-only session cookie. The current requirements declare a service address as required, an `Idempotency-Key` as required for retry safety, slot selection as preferred, and a maximum of 12 checklist items. The manifest also advertises customer portal preferences through `customerPortalEndpoints.preferences`, customer messaging and locations, estimate approval through `estimateEndpoints.approve`, reviews through `reviewEndpoint`, technician field operations through `capabilities.technicianFieldOperations`, and encrypted offline replay through `capabilities.technicianOfflineReplay`. Future landing-page agents should consume this manifest rather than duplicating service-route assumptions.
...
The signed technician workspace is documented in the canonical OpenAPI contract. A technician token can read `/api/public/technician-job` and `/api/public/technician-job/field-guide`, advance `/status`, read or mutate `/clock`, record `/location`, and complete `/complete`; every mutation should send an `Idempotency-Key`. The existing field workflow also supports checklist completion, photos, findings, forms, labor, materials, assets, payments, customer context, visit status, and field estimates. Keep this mobile surface tenant-scoped, rate-limited, and free of owner credentials.
