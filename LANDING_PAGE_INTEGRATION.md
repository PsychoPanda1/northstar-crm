...
Landing pages should resolve their service identity from `GET /api/public/tenant?service=plumbing`, then fetch `GET /api/public/availability?service=plumbing` to render `slotOptions` (`id`, display `label`, UTC `startsAt`/`endsAt`, and IANA `timeZone`). The legacy `slots` label array remains available. Submit a selected slot to `POST /api/public/bookings?service=plumbing` with `{ name, phone, email, location, requestedService, slotId, time, source, checklist, website: '' }` and an `Idempotency-Key`; `slotId` is preferred and `time` remains a compatibility fallback. `checklist` is optional: when supplied, it may contain up to 12 short labels that become the technician's required completion steps for that booked job; omitted or invalid checklists use the safe default. A successful booking returns `201` with a tenant-scoped job ID, normalized appointment timestamps, and short-lived `customerPortalToken`; retries with the same key return `200` and `duplicate: true`. Availability removes slots occupied by active tenant jobs, so the landing page and booking validation share the same capacity check. The current adapter supplies demo capacity; production should connect live timezone-aware availability and technician capacity.
