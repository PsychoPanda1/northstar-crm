...
3. Copy `landing-page-client.js` into the landing page, construct `NorthstarLandingClient({ service: '<key>', apiBase: '<crm-origin>' })`, and use `manifest()`, `tenant()`, `publicContact()`, `catalog()`, `availability()`, `ownerPortalUrl()`, `ownerPasswordLogin()` when `integration.ownerAuthMethods` includes `password`, `ownerOidcLogin()` when it includes `oidc`, `submitLead()`, and `book()` rather than duplicating endpoint paths.
