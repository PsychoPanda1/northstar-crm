...
The tenant discovery response includes `integration.version: 2`, copyable `bookingPath`, `ownerPortalPath`, `leadEndpoint`, `bookingEndpoint`, and `availabilityEndpoint` values, customer-portal action endpoints, capability flags, and `bookingRequirements`. The current requirements declare a service address as required, an `Idempotency-Key` as required for retry safety, slot selection as preferred, and a maximum of 12 checklist items. The manifest also advertises customer portal preferences through `customerPortalEndpoints.preferences`. Future landing-page agents should consume this manifest rather than duplicating service-route assumptions.
