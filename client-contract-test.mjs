...
assert(server.includes('statusIdempotencyKey') && repository.includes('updateLeadStatus(id, status, note = \'\', idempotencyKey') && app.includes('data-lead-status-action'), 'lead status workflow is not wired');
...
assert(server.includes('allowPublicMutation') && server.includes('publicMutationScope') && server.includes("pathname.startsWith('/api/public/technician-job')") && server.includes("rateLimited(res, 'rate_limited')"), 'public portal mutations do not have bounded abuse protection');
...
assert(technician.includes('northstar.webmanifest') && technician.includes("serviceWorker.register('/northstar-sw.js'") && ownerManifest.includes('"display": "standalone"') && ownerManifest.includes('"start_url": "/portal"') && northstarServiceWorker.includes("'northstar-shell-v2'") && northstarServiceWorker.includes("caches.delete(key)") && northstarServiceWorker.includes("cache: 'no-store'") && northstarServiceWorker.includes("'/technician.html'") && northstarServiceWorker.includes("'/portal'") && northstarServiceWorker.includes("url.pathname === '/technician.html' ? '/technician.html' : '/index.html'") && !northstarServiceWorker.includes('api/') , 'Northstar workspaces are not safely installable');
...
assert(app.includes("type === 'leads' ? ([item.location, item.note].filter(Boolean).join(' · ') || item.source)") && server.includes("const location = String(body.location || '').trim()") && server.includes("const note = String(body.message || body.note || '').trim()"), 'landing-page lead context is not preserved in the owner inbox');
