...
assert(app.includes('message-dispatch') && app.includes('dispatchMessages') && repository.includes('/api/integrations/messages/dispatch') && server.includes('MESSAGE_PROVIDER_URL') && server.includes('provider_timeout') && server.includes('messageProvider: messageProviderConfigured()'), 'server-side message provider dispatch is not wired');
