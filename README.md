...
The image runs as the unprivileged `node` user in production mode, stores transactional state in `/app/data/northstar.sqlite` using Node's SQLite runtime, and exposes `/api/ready` as its container readiness check. Readiness requires a non-default session secret, configured owner/staff authentication or at least one provisioned OIDC owner, and all five webhook secrets (`NORTHSTAR_PAYMENT_WEBHOOK_SECRET`, `NORTHSTAR_MESSAGE_WEBHOOK_SECRET`, `NORTHSTAR_CALL_WEBHOOK_SECRET`, `NORTHSTAR_FINANCING_WEBHOOK_SECRET`, and `NORTHSTAR_FLEET_WEBHOOK_SECRET`); configure these through deployment environment variables and do not bake credentials into the image. Demo login is disabled in production unless `NORTHSTAR_ALLOW_DEMO_LOGIN=true` is explicitly set for an isolated preview or smoke environment. The container release workflow runs the complete application verification suite, builds a verification image, and checks both `/api/ready` and the Docker healthcheck before publishing. Local development continues to use the JSON adapter unless `NORTHSTAR_SQLITE_FILE` is set.
