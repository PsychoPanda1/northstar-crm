...
For a repeatable single-host deployment, use the included `docker-compose.yml`: copy `.env.example` to `.env`, replace every placeholder, then run `docker compose up --build -d`. Compose requires the session, owner, tenant, and webhook secrets before starting, mounts the SQLite database and recovery snapshot on the named `northstar-data` volume, and exposes the same `/api/ready` health gate. This is suitable for a single host; horizontally scaled production still requires managed shared storage and coordinated session strategy.
