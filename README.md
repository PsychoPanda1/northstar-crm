...
Set `NORTHSTAR_REQUIRE_LIVE_PROVIDERS=true` for a production deployment that must not become ready until the server-side lead, inventory, accounting, message, payment, and document provider URLs are configured. Leave it `false` only for an intentional preview or a deployment that does not yet accept provider-backed communications, warehouse synchronization, accounting synchronization, document delivery, or payments; `/api/integrations/health` still reports each provider's live configuration status to authenticated staff. Approved payroll periods can optionally be handed off through `NORTHSTAR_PAYROLL_PROVIDER_URL` and `POST /api/integrations/payroll/dispatch`, with bounded retries and stable idempotency keys.
