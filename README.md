...
Available local API routes include `GET /api/health`, `POST /api/auth/demo-login`, `GET /api/session`, `GET /api/dashboard`, `GET /api/customers`, `GET /api/leads`, `GET /api/estimates`, `GET /api/invoices`, `GET /api/plans`, `GET /api/dispatch`, `POST /api/leads`, `POST /api/jobs`, `POST /api/jobs/:id/assign`, `POST /api/jobs/:id/status`, `POST /api/plans`, `POST /api/plans/:id/renew`, `POST /api/estimates`, `POST /api/estimates/:id/approve`, `POST /api/invoices`, `POST /api/invoices/:id/pay`, `POST /api/tasks/:index`, and `POST /api/actions`. The demo API uses an HMAC-signed, short-lived token and persists local development state in the ignored `.northstar-data.json`; production must use a real identity provider and durable tenant-scoped storage.
