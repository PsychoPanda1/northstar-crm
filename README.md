...
For multi-business deployments, `NORTHSTAR_TENANT_PROVIDER_CONFIG_JSON` can override the lead, message, inventory, accounting, payment, document, and payroll provider URL/API key per tenant, for example `{ "clearwater-plumbing": { "lead": { "url": "https://leads.example/ingest", "apiKey": "server-secret" }, "message": { "url": "https://messages.example/send", "apiKey": "server-secret" }, "inventory": { "url": "https://warehouse.example/sync", "apiKey": "server-secret" }, "accounting": { "url": "https://erp.example/sync", "apiKey": "server-secret" }, "payment": { "url": "https://payments.example/intents", "apiKey": "server-secret" }, "document": { "url": "https://documents.example/deliver", "apiKey": "server-secret" }, "payroll": { "url": "https://payroll.example/runs", "apiKey": "server-secret" } } }`. This value is server-only and never returned in manifests, snapshots, audit metadata, or browser payloads. Global provider variables remain the fallback for tenants without an override; production rejects non-HTTPS external provider URLs.
