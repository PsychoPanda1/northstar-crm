...
- Generated pre-job briefs for technicians with scoped customer history, known equipment, prior findings, priorities, and service-specific checks
...
- Optional server-side automation worker runs appointment, estimate, receivables, renewal, and configured lead-, inventory-, and accounting-provider delivery work on a bounded cadence with deduplication, retry limits, opt-out enforcement, and audit evidence
...
Set `NORTHSTAR_REQUIRE_LIVE_PROVIDERS=true` for a production deployment that must not become ready until the server-side lead, inventory, accounting, message, and payment provider URLs are configured. Leave it `false` only for an intentional preview or a deployment that does not yet accept provider-backed communications, warehouse synchronization, accounting synchronization, or payments; `/api/integrations/health` still reports each provider's live configuration status to authenticated staff.
