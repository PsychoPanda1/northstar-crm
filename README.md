...
Set `NORTHSTAR_REQUIRE_LIVE_PROVIDERS=true` for a production deployment that must not become ready until the server-side lead, inventory, message, and payment provider URLs are configured. Leave it `false` only for an intentional preview or a deployment that does not yet accept provider-backed communications, warehouse synchronization, or payments; `/api/integrations/health` still reports each provider's live configuration status to authenticated staff.
