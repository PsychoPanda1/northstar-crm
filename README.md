...
Additional current routes include `POST /api/auth/login`, `POST /api/auth/refresh`, `GET /api/integrations/health`, `GET /api/public/tenant`, `GET /api/public/availability`, `POST /api/public/bookings`, `GET /api/dispatch/route-manifest`, `POST /api/dispatch/reminders`, `POST /api/messages/:id/retry`, `GET /api/jobs/:id`, `GET /api/jobs/:id/calendar`, `POST /api/team`, and `GET|POST /api/invoices/:id/schedule`. See [PORTAL_CONTRACT.md](PORTAL_CONTRACT.md) for request shapes, role boundaries, and customer-safe response rules.
