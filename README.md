...
See [PARITY_MATRIX.md](PARITY_MATRIX.md) for the maintained ServiceTitan/Jobber-style capability map, current evidence, and remaining production work. Owners can save reusable, tenant-scoped report views through `/api/reports/custom/views`, compare current and prior periods through `/api/reports/custom/compare`, and schedule daily, weekly, or monthly aggregate CSV delivery through `/api/reports/custom/schedules` so recurring service operators can return to the same KPI and date-window definition.
