...
Set `NORTHSTAR_REQUIRE_LIVE_PROVIDERS=true` for a production deployment that must not become ready until the server-side lead, inventory, accounting, message, payment, document, and payroll provider URLs are configured for every configured production tenant. Global provider URLs remain the fallback; `NORTHSTAR_TENANT_PROVIDER_CONFIG_JSON` overrides them per tenant, and readiness now evaluates the effective provider for each tenant instead of checking only the global fallback. Malformed overrides, unknown tenant IDs, unsupported provider kinds, credential-bearing URLs, and overlong API keys fail the tenant-provider readiness check instead of being silently ignored. Leave it `false` only for an intentional preview or a deployment that does not yet accept provider-backed communications, warehouse synchronization, accounting synchronization, document delivery, payroll handoff, or payments; `/api/integrations/health` still reports each provider's live configuration status to authenticated staff. Approved payroll periods can optionally be handed off through `NORTHSTAR_PAYROLL_PROVIDER_URL` and `POST /api/integrations/payroll/dispatch`, with bounded retries and stable idempotency keys.

For multi-business deployments, `NORTHSTAR_TENANT_PROVIDER_CONFIG_JSON` can override the lead, message, inventory, accounting, payment, and payroll provider URL/API key per tenant, for example `{ "clearwater-plumbing": { "lead": { "url": "https://leads.example/ingest", "apiKey": "server-secret" }, "message": { "url": "https://messages.example/send", "apiKey": "server-secret" }, "inventory": { "url": "https://warehouse.example/sync", "apiKey": "server-secret" }, "accounting": { "url": "https://erp.example/sync", "apiKey": "server-secret" }, "payment": { "url": "https://payments.example/intents", "apiKey": "server-secret" }, "payroll": { "url": "https://payroll.example/runs", "apiKey": "server-secret" } } }`. This value is server-only and never returned in manifests, snapshots, audit metadata, or browser payloads. Global provider variables remain the fallback for tenants without an override.
