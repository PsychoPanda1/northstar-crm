...
Additional current routes include `POST /api/auth/login`, `POST /api/auth/refresh`, `GET /api/integrations/health`, `GET /api/public/tenant`, `GET /api/public/availability?days=...` (up to a 14-day weekday booking window), `POST /api/public/bookings`, `POST /api/public/customer-portal/reschedule`, `POST /api/public/job-status/reschedule`, `POST /api/public/estimate/decline`, `GET /api/dispatch/route-manifest`, `GET /api/dispatch/route-summary`, `GET /api/dispatch/route-calendar`, `POST /api/dispatch/reminders`, `POST /api/estimates/reminders`, `POST /api/customers/reactivation`, `GET /api/reports/marketing`, `GET /api/reports/technicians`, `POST /api/messages/:id/retry`, `POST /api/payment-intents/:id/retry`, `GET /api/jobs/:id`, `GET /api/jobs/:id/calendar`, `GET /api/invoices/:id/receipt`, `POST /api/team`, and `GET|POST /api/invoices/:id/schedule`. See [PORTAL_CONTRACT.md](PORTAL_CONTRACT.md) for request shapes, role boundaries, and customer-safe response rules.
