(() => {
  ...
  const setMode = (nextMode, count) => {
    ...
    message.textContent = '';
    dialog.querySelector('h2').textContent = mode === 'assign' ? 'Assign selected jobs' : 'Update selected job status';
  };
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      message.textContent = mode === 'assign' ? (error?.message === 'bulk assignment failed' ? 'Assignment rejected. Check technician skills and schedule conflicts.' : 'Bulk assignment is unavailable.') : 'Status update failed. Check lifecycle transitions and technician assignments.';
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!assign && !status) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    void open(status ? 'status' : 'assign');
  }, true);
})();
